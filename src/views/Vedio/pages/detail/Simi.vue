    ...
    <dl class="simi-lists">
      ...
      >
        <div class="simi-item-left">
          <a-avatar class="simi-item-img" shape="square" icon="user" :src="item.coverUrl" />
          <span class="simi-item-playTime">
            <AIconfont type="icon-up1-copy" />
            <span>{{transformW(item.playTime)}}</span>
          </span>
        </div>
        <div class="simi-item-right">
          <span class="simi-item-right-title">{{item.title}}</span>

          <span class="simi-item-right-name">by {{filterName(item.creator)}}</span>
        </div>
      </dd>
    </dl>
  </div>
...
export default class Radio extends Vue {
  ...
  public filterName(args: any) {
    let result = '';
    if (args) {
      const temp = args.map((e: any) => e.userName);
      if (temp) { result = temp.toString(); }
    }
    return result;
  }
}
